...
def compute_foreground_ratio(image_type='W', img_size=(256, 256)):
    data = load_original_data(image_type=image_type)
    dataset = CellSegmentationDataset(data['image_paths'], data['mask_paths'], img_size=img_size)
    
    total_pixels = 0
    foreground_pixels = 0

    for _, mask in tqdm(dataset, desc="Computing foreground ratio"):
        mask_np = mask.squeeze().numpy()  # shape: [H, W]
        foreground_pixels += mask_np.sum()
        total_pixels += mask_np.size

    ratio = foreground_pixels / total_pixels
    print(f"Foreground (cell) pixels: {ratio*100:.2f}%")
    return ratio
